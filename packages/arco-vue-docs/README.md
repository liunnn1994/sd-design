...
5. 不要手工修改 `packages/arco-vue-docs/components/**` 和 `.vitepress/theme/generated/**`，这些文件会在 `docs:prepare` 时被重建。
...
```bash
pnpm --filter @arco-design/arco-vue-docs run docs:prepare
pnpm --filter @arco-design/arco-vue-docs run docs:dev
pnpm --filter @arco-design/arco-vue-docs run docs:build
```
