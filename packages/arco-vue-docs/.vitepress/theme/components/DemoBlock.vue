...
const importMap = {
  imports: {
    ...
    '@arco-design/web-vue': `https://esm.sh/@arco-design/web-vue@${ARCO_VERSION}?external=vue`,
    '@arco-design/web-vue/es/icon': `https://esm.sh/@arco-design/web-vue@${ARCO_VERSION}/es/icon?external=vue`,
    '@arco-design/web-vue/': `https://esm.sh/@arco-design/web-vue@${ARCO_VERSION}/`,
  },
};

const previewOptions = computed(() => ({
  headHTML: [
    `<link rel="stylesheet" href="https://unpkg.com/@arco-design/web-vue@${ARCO_VERSION}/dist/arco.css">`,
    '<style>body{margin:0;padding:16px;}body[arco-theme="dark"]{background:#141414;color:#f2f3f5;}<\/style>',
    `<script>window.__ARCO_THEME__=${JSON.stringify(replTheme.value)};document.addEventListener('DOMContentLoaded',function(){if(window.__ARCO_THEME__==='dark'){document.body.setAttribute('arco-theme','dark');}else{document.body.removeAttribute('arco-theme');}});<\/script>`,
  ].join(''),
  customCode: {
    importCode: `import ArcoVue from '@arco-design/web-vue';\nimport ArcoVueIcon from '@arco-design/web-vue/es/icon';`,
    useCode: `app.use(ArcoVue);\napp.use(ArcoVueIcon);\nif (window.__ARCO_THEME__ === 'dark') { document.body.setAttribute('arco-theme', 'dark'); } else { document.body.removeAttribute('arco-theme'); }`,
  },
}),);
