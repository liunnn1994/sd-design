...
局部主题通过独立浮层容器同步到 Popover、Tooltip、Select 等浮层；使用 `global` 时则对整个页面生效。
...
- seed 定义基础设计，自动派生色阶和尺寸；algorithm 可组合暗色、紧凑模式
...
使用 [在线主题编辑器](/guides/theme-editor/) 调整基础、全局和组件主题，查看真实文档示例并导出可直接使用的 JSON。
