...
`floating-options` 会原样传给 `@floating-ui/vue` 的 `useFloating()`。它与旧定位属性冲突时以 Floating UI 配置为准；`middleware` 会完整替换默认中间件。详见 [Floating UI 迁移指南](/guides/floating-ui/)。
...
| floating-options | 原样传给 Floating UI `useFloating()` 的配置，显式字段优先于旧定位属性 | `FloatingOptions` | `-` |  |
