...
搜索不区分大小写，会匹配选项值、`label` 和 `keywords`。过滤后没有匹配选项的分组会自动隐藏，所有分组均无匹配项时显示 `ModelSelectorEmpty`。搜索时有匹配项的分组会临时展开，清空搜索后恢复搜索前的展开状态。点击分组标题可独立展开或收起；`default-expanded={false}` 可让所有分组初始收起，默认全部展开。收起的模型不会参与键盘导航或快捷键选择。
...
| default-expanded      | 分组是否默认全部展开     | `boolean` | `true`  |
...
`ModelSelectorDialog` 同时接受 `ModelSelector` 的 `visible`、`default-visible`、`default-expanded`、`close-on-select`、`reset-query-on-close`，以及 `ModelSelectorContent` 的全部 props。它同样触发 `visible-change` 和 `select`。
