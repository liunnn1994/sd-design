...
- Copy checkpoint: 27 passing (18 behavior + 4 demos + 2 async lifecycle + 2 dynamic-label + 1 real clipboard), retries disabled. Both new regressions reproduced an incorrect copy-event payload after content changes during an outstanding write and a success notification after unmount. The operation now captures its content and skips UI feedback after disposal. The original 24-case Chrome run used stubbed clipboard APIs. Latest full run passes all 27 cases, including a real navigator.clipboard write/read with Chrome test permissions and focus emulation restored afterward. Existing failure-path specs intentionally suppress the component copy-failure rejection and produce verbose Vue warnings. Removing the visible text slot reproduced loss of the icon-only accessible name. Slot presence now synchronizes into reactive state before updates so the nested Tooltip content refreshes; both directions and changed consumer aria-label pass. The final reverse-direction test initially chained a text assertion onto an undefined attribute subject; splitting DOM queries corrected the test. Review remains in progress for overlapping operations. Standard TypeScript validation remains pending.

Includes every immediate directory in components; internal helpers and styles require shared-support review rather than pretending they are public components. Next public component: copy.
