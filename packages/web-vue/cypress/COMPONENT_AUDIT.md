...
- Cascader checkpoint: 74 passing (45 behavior + 18 demos + 2 readonly + 8 lazy-load lifecycle + 1 search lifecycle), retries disabled. Deduplicate pending expansion loads, ignore results after the source option is replaced or unmounted, and release loading state after synchronous throws or rejected Promises so expansion can retry. Both Cascader and CascaderPanel reproduced duplicate loads and stale results before fixes. Search now cancels its pending timer on unmount; the regression checks the actual timer identity because Vue suppresses emitted events after unmount and a callback-only assertion falsely passed. Full Chrome component suite passes together. Component review remains in progress: dynamic configuration, keyboard branches, cache invalidation and remaining style/utils coverage still need review. Standard TypeScript validation remains pending with the pre-existing TNB toolchain.
