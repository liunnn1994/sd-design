...
- Copy checkpoint: 24 passing (18 behavior + 4 demos + 2 async lifecycle), retries disabled. Both new regressions reproduced an incorrect copy-event payload after content changes during an outstanding write and a success notification after unmount. The operation now captures its content and skips UI feedback after disposal. Full Chrome component run passed; clipboard APIs are stubbed in these tests. Existing failure-path specs intentionally suppress the component copy-failure rejection and produce verbose Vue warnings. Review remains in progress for actual clipboard integration, overlapping operations and dynamic accessible names. Standard TypeScript validation remains pending.
