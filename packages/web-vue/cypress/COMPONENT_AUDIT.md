...
- Cropper checkpoint: 22 passing (17 behavior + 4 demos + 1 dynamic fitting), retries disabled. Read component lifecycle, prop synchronization, public types, installation and styles. The new browser regression uses a loaded SVG and real CropperImage/CropperSelection elements; enabling fitSelectionToImage after mount previously left a manually set selection unchanged. A post-render watcher now schedules fitting and the alignment callback checks the current enabled state. Full component run passes. Review remains in progress for removed child props, loading races, controlled geometry and pointer interactions. Standard TypeScript validation remains pending.
