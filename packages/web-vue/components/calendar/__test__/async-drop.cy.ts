...
describe('Calendar asynchronous drop', () => {
  for (const deleteEnabled of [true, false]) {
    for (const accepted of [true, false, 'error'] as const) {
      it(`preserves one event when the drop result is ${accepted} after dragend (delete=${deleteEnabled})`, () => {
        ...
        cy.mount(
          defineComponent({
            ...
            setup: () => ({
              onDrop,
              editableEvents: { drag: true, delete: deleteEnabled },
              events: ref([
                { start: '2025-01-08 10:00', end: '2025-01-08 11:00', title: 'Transfer' },
              ]),
            }),
            template:
              '<Calendar id="source" view="day" view-date="2025-01-08" :events="events" :editable-events="editableEvents" style="height:300px"/><Calendar id="destination" view="day" view-date="2025-01-08" :editable-events="true" @event-drop="onDrop" style="height:300px"/>',
          }),
        );
        ...
        cy.get(
          `${accepted === true ? '#destination' : '#source'} .sd-calendar__event-title`,
