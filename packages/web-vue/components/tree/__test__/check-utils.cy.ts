...
describe('checkUtils', () => {
  const data = [
    {
      ...
      children: [
        { title: 'Leaf', key: '0-0-1' },
        {
          ...
          children: [{ title: 'Leaf', key: '0-0-2-1' }],
        },
      ],
    },
  ];
  ...
  it('does not check nodes that are disabled or have disableCheckbox', () => {
    ...
    expect(checkedResult.length).to.equal(2);
    ...
    expect(checkedResult.length).to.equal(0);
  });

  it('only checks itself when checkStrictly is true', () => {
    let [checkedResult] = getCheckedStateByCheck({
      node: checkedNode,
      checked: true,
      checkedKeys: [],
  ...
  it('[getCheckedStateByInitKeys] skips disabled/disableCheckbox nodes', () => {
    const [result] = getCheckedStateByInitKeys({ initCheckedKeys: ['0-0'], key2TreeNode });
    expect(result.length).to.equal(2);
  });

  it('[getCheckedStateByInitKeys] only checks itself when checkStrictly is true', () => {
  ...
  it('[getCheckedStateByInitKeys] only checks leaves when onlyCheckLeaf is true', () => {
  ...
  it('stops updating the parent when a node is disabled', () => {
