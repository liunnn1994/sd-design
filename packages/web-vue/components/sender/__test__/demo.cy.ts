...
describe('<sender> interactive demos:', () => {
  ...
  it('runs the real AudioWorklet capture graph through repeated RichTextEditor finalization cycles', () => {
    ...
    for (let cycle = 1; cycle <= 3; cycle += 1) {
      ...
      cy.get('button[aria-label="停止语音输入"]').then(($button) => {
        $button[0].click();
        $button[0].click();
      });
      cy.get('[data-cy="speech-status"]').should('contain.text', '等待后端完成');
      cy.get('[data-cy="speech-cycles"]').should('contain.text', `启停次数：${cycle}`);
      cy.get('.sd-rich-text-editor-content').should('have.attr', 'contenteditable', 'false');

      cy.contains('button', '模拟中间结果').click();
      cy.get('.sd-rich-text-editor-content').should('contain.text', '语音片段');
      cy.contains('button', '模拟完成').click();
      cy.get('[data-cy="speech-status"]').should('contain.text', '后端处理完成');
      cy.get('.sd-rich-text-editor-content').should('have.attr', 'contenteditable', 'true');
    }

    cy.get('[data-cy="speech-context"]').should('contain.text', 'AudioContext：1');
    cy.get('@getUserMedia').should('have.callCount', 3);
    cy.contains('button', '模拟中间结果').should('be.disabled');
  });
});
