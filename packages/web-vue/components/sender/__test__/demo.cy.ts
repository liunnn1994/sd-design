...
interface AudioWorkletLifecycle {
  created: number;
  stopped: number;
  closed: number;
}
...
/**
 * 记录真实 AudioWorklet 节点的停止确认和端口关闭，不替换节点或 processor。
 * @returns 当前页面的 AudioWorklet 生命周期计数
 */
const installAudioWorkletLifecycleProbe = (): AudioWorkletLifecycle => {
  const lifecycle: AudioWorkletLifecycle = { created: 0, stopped: 0, closed: 0 };
  cy.window().then((win) => {
    const OriginalAudioWorkletNode = win.AudioWorkletNode;
    const WrappedAudioWorkletNode = new Proxy(OriginalAudioWorkletNode, {
      construct(target, argumentsList, newTarget) {
        const node = Reflect.construct(target, argumentsList, newTarget) as AudioWorkletNode;
        lifecycle.created += 1;
        node.port.addEventListener('message', (event) => {
          if (event.data?.type === 'flushed') lifecycle.stopped += 1;
        });
...
describe('<sender> interactive demos:', () => {
  ...
  // Generated by Cypress Author on 2026-07-29.
  it('反复启停真实语音采集 → 每轮 AudioWorklet processor 都终止', () => {
    const lifecycle = installAudioWorkletLifecycleProbe();
