...
export default function usePickerState(props: PickerStateProps) {
  ...
  const computedModelValue = computed(() => {
    return getDayjsValue(modelValue.value, format.value, utcOffset?.value, timezone?.value);
  });

  const computedDefaultValue = computed(() => {
    return getDayjsValue(defaultValue.value, format.value, utcOffset?.value, timezone?.value);
  });
