    ...
    .map((c) => {
      ...
      const smallerSize = c.size
        ...
        .map((s) => {
          ...
          return `calc(${Math.round(val * 0.9)}px * var(--beam-density, 1))`;
        })
        .join(' ');
...
function getLineBloomGradients(
  ...
  if (isDark) {
    return `radial-gradient(ellipse calc(${thinW1} * var(--beam-density, 1) * var(--beam-spike-${id})) calc(${thinH1} * var(--beam-density, 1) * var(--beam-h-${id})) at 8% calc(100% - 2px), ${sc1}, ${sc1_mid} 30%, transparent 88%),
    ...
    return `radial-gradient(ellipse calc(${thinW1} * var(--beam-density, 1) * var(--beam-spike-${id})) calc(${thinH1} * var(--beam-density, 1) * var(--beam-h-${id})) at 8% calc(100% - 2px), ${sc1}, ${sc1_lt} 30%, transparent 88%),
       radial-gradient(ellipse calc(10px * var(--beam-density, 1) * var(--beam-spike2-${id})) calc(35px * var(--beam-density, 1) * var(--beam-h-${id})) at 22% calc(100% - 4px), ${sc2}, ${sc2_lt} 50%, transparent 95%),
       radial-gradient(ellipse calc(${thinW2} * var(--beam-density, 1) * (2 - var(--beam-spike-${id}))) calc(${thinH2} * var(--beam-density, 1) * var(--beam-h-${id})) at 36% calc(100% - 3px), ${spikes[0].color1}, ${spikes[0].color2} 40%, transparent 90%),
       radial-gradient(ellipse calc(14px * var(--beam-density, 1) * var(--beam-spike2-${id})) calc(28px * var(--beam-density, 1) * var(--beam-h-${id})) at 50% calc(100% - 2px), ${spikes[1].color1}, ${spikes[1].color2} 55%, transparent 96%),
       radial-gradient(ellipse calc(${thinW3} * var(--beam-density, 1) * (2 - var(--beam-spike2-${id}))) calc(${thinH3} * var(--beam-density, 1) * var(--beam-h-${id})) at 64% calc(100% - 4px), ${spikes[2].color1}, ${spikes[2].color2} 35%, transparent 89%),
       radial-gradient(ellipse calc(7px * var(--beam-density, 1) * var(--beam-spike-${id})) calc(45px * var(--beam-density, 1) * var(--beam-h-${id})) at 78% calc(100% - 2px), ${spikes[3].color1}, ${spikes[3].color2} 48%, transparent 94%),
...
function pulseInnerGradients(variant: BorderBeamColorVariant, id: string, isDark: boolean): string {
  ...
  const cornerGrads = corners.map(
    ([x, y, q]) =>
      `radial-gradient(ellipse calc(60px * var(--beam-density, 1)) calc(60px * var(--beam-density, 1)) at ${x} ${y}, rgba(${cornerRGB}, calc(${cornerAlpha} * var(--bop-${q}-${id}))), transparent 70%)`,
  );
...
// ── Pulse breathing driver helpers ───────────────────────────────────────
...
function generatePulseOuterVariantCSS(options: GenerateStylesOptions): string {
  ...
  const strokeBackground =
    hairlineOpacity > 0 ? `${strokeGradients},\n    ${hairlineLine}` : strokeGradients;
