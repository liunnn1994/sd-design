...
class Lexer {
  ...
  unicodeMode = false;
  constructor(regex: string, escapeBackslash: boolean, unicodeMode = false) {
    this.regex = regex;
  ...
  public read(): Token {
    ...
    switch (char) {
      ...
      default: {
        const type = dict.token.get(char);
        if (type) {
          return this.token(type);
        }
        // 原生 RegExp 校验已经排除非法语法；这里保证未知字符仍能向前推进。
  ...
  readBackslash(range = false): Token {
    ...
    if (this.unicodeMode) {
      // Unicode 模式下整体识别码点和属性转义；非 Unicode 模式保持上游的普通字符语义。
      if (
        this.readByRegex(patterns.unicodeCodePoint) ||
        this.readByRegex(patterns.unicodeProperty)
      ) {
        return {
          type: TokenType.CharacterClass,
          span: { start, end: this.index },
  ...
  readNormalCharacters(): Token | null {
    ...
    const matches = curRegex.match(patterns.specialCharacter);
    if (matches) {
      if (matches.index) {
        return {
