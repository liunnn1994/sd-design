<template>
  <VNodeRenderer :content="render()" />
</template>

<script setup lang="ts">
  import type { ComponentPublicInstance, CSSProperties, PropType, Slot, VNodeChild } from 'vue';
  import {
    computed,
    inject,
    onMounted,
  ...
  const loadMore = toRef(props, 'loadMore');
  const filterIconAlignLeft = toRef(props, 'filterIconAlignLeft');
  const selectedKeys = toRef(props, 'selectedKeys');
  const defaultSelectedKeys = toRef(props, 'defaultSelectedKeys');
  const expandedKeys = toRef(props, 'expandedKeys');
  ...
  const { children, components: columnComponents } = useChildrenComponents('TableColumn');
  ...
  watch([columnComponents, slotColumnMap], ([componentIds, slotColumnMap]) => {
  ...
  const handleCellContextmenu = (record: TableDataWithRaw, column: TableColumnData, ev: Event) => {
  ...
  const tableGridTemplate = computed(() =>
    getTableGridTemplate(dataColumns.value, operations.value, columnWidth),
  );
  const getTableElementStyle = (base?: CSSProperties): CSSProperties =>
    ({
      ...base,
      '--sd-table-grid-template': tableGridTemplate.value,
    }) as CSSProperties;
    ...
    return _createVNode(
      ...
      [
        slots['expand-icon']?.({
          ...
          (expanded ? _createVNode(IconMinus, null, null) : _createVNode(IconPlus, null, null)),
      ],
    );
  };
  const renderExpand = (
  ...
  const renderBody = () => {
    ...
    return _createVNode(Tbody, null, {
      default: () => [
                  ...
                  operations.value.map((operation, index) =>
                    _createVNode(
                      OperationTh,
                      {
                        ...
                        ref: (ins: Element | ComponentPublicInstance | null) =>
                          setThRef(operation.name, ins),
                        operationColumn: operation,
                        operations: operations.value,
                        selectAll: Boolean(
                ...
                : _createVNode(
                    ...
                    _mergeProps(
                      ...
                      displayScrollbar.value
                        ? {
                            outerStyle: bodyScrollbarOuterStyle,
                            ...bodyScrollbarRestProps,
                          }
                        : {},
                      {
                        onScroll: onTbodyScroll,
  ...
  const renderTable = (content?: Slot) => {
    ...
    const { outerStyle: contentOuterStyle, ...contentScrollbarRestProps } = scrollbarProps.value;
    const contentScrollbarOuterStyle = Array.isArray(contentOuterStyle)
      ? [
          {
            height: '100%',
          },
        ...
        [
          _createVNode(
            ...
            _mergeProps(
              ...
              scrollbar.value && !splitTable.value
                ? {
                    outerStyle: contentScrollbarOuterStyle,
                    ...contentScrollbarRestProps,
                  }
                : {},
              {
  ...
  const render = () => {
    ...
    return _createVNode(
      ...
      [
        children.value,
        _createVNode(Spin, mergedSpinProps.value, {
          default: () => [
  ...
  defineExpose({
    ...
    resetSorters,
    clearSorters,
    scrollIntoView: (options: ScrollIntoViewOptions) => virtualListRef.value?.scrollTo(options),
  });
</script>
