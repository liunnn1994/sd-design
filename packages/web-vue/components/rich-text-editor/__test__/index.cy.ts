...
describe('RichTextEditor', () => {
  ...
  it('连续纠正短文本 → 最终文本与最新识别结果一致', () => {
    ...
    cy.get('@vue').then(({ wrapper }) => {
      const editor = getEditor(wrapper);
      const candidates = ['张', '张三', '张山', '张珊', '张珊。'];
      let expected = '';
      for (let round = 0; round < 200; round += 1) {
        let applied = '';
        for (const candidate of candidates) {
          const appended = candidate.startsWith(applied);
          const inserted = appended ? candidate.slice(applied.length) : candidate;
          editor.insertText(inserted, {
            position: 'end',
            replaceCharacters: appended ? undefined : applied,
          });
          expected = appended
            ? `${expected}${inserted}`
            : `${expected.slice(0, -applied.length)}${candidate}`;
          applied = candidate;
        }
        editor.insertText('\n', { position: 'end' });
        expected += '\n';
      ...
      expect(changeEvents).to.equal(1201);
    });
  });
