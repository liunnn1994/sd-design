...
- 文档站的 `src/components/generated/` 是生成产物，通常不要手改
...
- 不要手改 `packages/sd-vue-docs/src/components/generated/` 这类生成目录，除非已经确认它在当前任务里就是人工维护源。
...
10. 写 `__test__/index.test.ts`。
11. 写 `__test__/demo.test.ts`。
12. 写 `packages/sd-vue-docs/src/content/docs/components/<component-name>/index.mdx`。
13. 核实 demo 相关链路是否需要额外同步。
14. 运行验证命令。
15. 修复和本次改动直接相关的错误。
...
3. 再跑模块构建
4. 最后跑 docs vendor / docs dev 验证文档侧接入
...
- 只写 MDX 页面，忘了 generated demo 不是手工源
