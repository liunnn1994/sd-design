...
Common deoptimization reasons: | Reason | Meaning | |--------|---------| | `not a Smi` | Expected small integer, got something else | | `wrong map` | Object shape changed | | `minus zero` | Result was -0 | | `out of bounds` | Array access beyond length | | `not a heap number` | Expected heap-allocated number |
