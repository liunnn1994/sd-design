...
The comparator signature is `(newSourceValue, cachedValue) => boolean`. When it returns `true`, the cache is kept as-is. When it returns `false`, the cache is updated to the new source value.
