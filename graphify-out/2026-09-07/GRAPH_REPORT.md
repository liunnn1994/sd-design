# Graph Report - sd-design  (2026-07-07)

## Corpus Check
- 0 files · ~0 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 234 nodes · 205 edges · 75 communities (17 shown, 58 thin omitted)
- Extraction: 100% EXTRACTED · 0% INFERRED · 0% AMBIGUOUS
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `a7143a49`
- Run `git rev-parse HEAD` and compare to check if the graph is stale.
- Run `graphify update .` after code changes (no API cost).

## Community Hubs (Navigation)
- [[_COMMUNITY_Icon System|Icon System]]
- [[_COMMUNITY_Component Entry Points & Icons|Component Entry Points & Icons]]
- [[_COMMUNITY_Layout & Panel Components|Layout & Panel Components]]
- [[_COMMUNITY_Select & Dropdown Logic|Select & Dropdown Logic]]
- [[_COMMUNITY_Type Utilities & Config|Type Utilities & Config]]
- [[_COMMUNITY_Icon-to-Component Wiring|Icon-to-Component Wiring]]
- [[_COMMUNITY_Color Picker System|Color Picker System]]
- [[_COMMUNITY_Anchor & Descriptions Components|Anchor & Descriptions Components]]
- [[_COMMUNITY_JSON Form Engine|JSON Form Engine]]
- [[_COMMUNITY_Input & Display Components|Input & Display Components]]
- [[_COMMUNITY_Date Picker & Panels|Date Picker & Panels]]
- [[_COMMUNITY_AvatarBadgeHooks|Avatar/Badge/Hooks]]
- [[_COMMUNITY_Ellipsis & Utility Hooks|Ellipsis & Utility Hooks]]
- [[_COMMUNITY_AgentSkill Evaluation|Agent/Skill Evaluation]]
- [[_COMMUNITY_Form & Tabs System|Form & Tabs System]]
- [[_COMMUNITY_Menu System|Menu System]]
- [[_COMMUNITY_InputTagEllipsis Utilities|Input/Tag/Ellipsis Utilities]]
- [[_COMMUNITY_BackTop & Affix Positioning|BackTop & Affix Positioning]]
- [[_COMMUNITY_Build Scripts|Build Scripts]]
- [[_COMMUNITY_Component Infrastructure|Component Infrastructure]]
- [[_COMMUNITY_Small Cluster 20|Small Cluster 20]]
- [[_COMMUNITY_Small Cluster 21|Small Cluster 21]]
- [[_COMMUNITY_Small Cluster 22|Small Cluster 22]]
- [[_COMMUNITY_Small Cluster 24|Small Cluster 24]]
- [[_COMMUNITY_Small Cluster 26|Small Cluster 26]]
- [[_COMMUNITY_Small Cluster 27|Small Cluster 27]]
- [[_COMMUNITY_Small Cluster 28|Small Cluster 28]]
- [[_COMMUNITY_Small Cluster 29|Small Cluster 29]]
- [[_COMMUNITY_Small Cluster 30|Small Cluster 30]]
- [[_COMMUNITY_Small Cluster 31|Small Cluster 31]]
- [[_COMMUNITY_Small Cluster 32|Small Cluster 32]]
- [[_COMMUNITY_Small Cluster 33|Small Cluster 33]]
- [[_COMMUNITY_Small Cluster 35|Small Cluster 35]]
- [[_COMMUNITY_Small Cluster 36|Small Cluster 36]]
- [[_COMMUNITY_Small Cluster 37|Small Cluster 37]]
- [[_COMMUNITY_Small Cluster 38|Small Cluster 38]]
- [[_COMMUNITY_Small Cluster 39|Small Cluster 39]]
- [[_COMMUNITY_Small Cluster 40|Small Cluster 40]]
- [[_COMMUNITY_Small Cluster 41|Small Cluster 41]]
- [[_COMMUNITY_Small Cluster 42|Small Cluster 42]]
- [[_COMMUNITY_Small Cluster 43|Small Cluster 43]]
- [[_COMMUNITY_Small Cluster 44|Small Cluster 44]]
- [[_COMMUNITY_Small Cluster 45|Small Cluster 45]]
- [[_COMMUNITY_Small Cluster 46|Small Cluster 46]]
- [[_COMMUNITY_Small Cluster 47|Small Cluster 47]]
- [[_COMMUNITY_Small Cluster 48|Small Cluster 48]]
- [[_COMMUNITY_Small Cluster 49|Small Cluster 49]]
- [[_COMMUNITY_Small Cluster 50|Small Cluster 50]]
- [[_COMMUNITY_Small Cluster 51|Small Cluster 51]]
- [[_COMMUNITY_Small Cluster 52|Small Cluster 52]]
- [[_COMMUNITY_Small Cluster 53|Small Cluster 53]]
- [[_COMMUNITY_Small Cluster 54|Small Cluster 54]]
- [[_COMMUNITY_Small Cluster 55|Small Cluster 55]]
- [[_COMMUNITY_Small Cluster 58|Small Cluster 58]]
- [[_COMMUNITY_Small Cluster 59|Small Cluster 59]]
- [[_COMMUNITY_Small Cluster 60|Small Cluster 60]]
- [[_COMMUNITY_Small Cluster 61|Small Cluster 61]]
- [[_COMMUNITY_Small Cluster 63|Small Cluster 63]]
- [[_COMMUNITY_Small Cluster 64|Small Cluster 64]]
- [[_COMMUNITY_Small Cluster 65|Small Cluster 65]]
- [[_COMMUNITY_Small Cluster 66|Small Cluster 66]]
- [[_COMMUNITY_Small Cluster 67|Small Cluster 67]]
- [[_COMMUNITY_Small Cluster 68|Small Cluster 68]]
- [[_COMMUNITY_Small Cluster 70|Small Cluster 70]]
- [[_COMMUNITY_Small Cluster 71|Small Cluster 71]]
- [[_COMMUNITY_Small Cluster 72|Small Cluster 72]]
- [[_COMMUNITY_Small Cluster 73|Small Cluster 73]]
- [[_COMMUNITY_Small Cluster 74|Small Cluster 74]]
- [[_COMMUNITY_Small Cluster 75|Small Cluster 75]]
- [[_COMMUNITY_Small Cluster 76|Small Cluster 76]]
- [[_COMMUNITY_Small Cluster 77|Small Cluster 77]]
- [[_COMMUNITY_Small Cluster 78|Small Cluster 78]]
- [[_COMMUNITY_Small Cluster 79|Small Cluster 79]]
- [[_COMMUNITY_Small Cluster 80|Small Cluster 80]]
- [[_COMMUNITY_Small Cluster 81|Small Cluster 81]]

## God Nodes (most connected - your core abstractions)

## Surprising Connections (you probably didn't know these)
- `Migrate Generated Demos to Script Setup + Tailwind` --modifies--> `Generated Demo Components`  [EXTRACTED]
   →   _Bridges community 10 → community 3_
- `Form Component Doc` --uses--> `DemoBlock component`  [EXTRACTED]
   →   _Bridges community 1 → community 0_
- `Ellipsis Component Docs` --uses--> `DemoBlock component`  [EXTRACTED]
   →   _Bridges community 6 → community 0_

## Import Cycles
- None detected.

## Communities (75 total, 58 thin omitted)

### Community 0 - "Icon System"
Cohesion: 0.11
Nodes (21): anchor component, anchor-link component, Anchor Component Docs, back-top component, BackTop Component Docs, calendar component, Calendar Event System, Calendar Component Docs (+13 more)

### Community 1 - "Component Entry Points & Icons"
Cohesion: 0.13
Nodes (21): Component Size System, Empty Component Doc, FieldRule type, form component, form-item component, Form Component Doc, useFormItem composable, useFormItem Hook (+13 more)

### Community 2 - "Layout & Panel Components"
Cohesion: 0.15
Nodes (19): ConfigProvider theme object, CSS variable fallback strategy, css-variable-token-system spec, docs-theme-editor spec, fix-local-theme design, fix-local-theme implementation baseline, fix-local-theme-implementation, fix-local-theme tasks (+11 more)

### Community 3 - "Select & Dropdown Logic"
Cohesion: 0.23
Nodes (16): Alert Component Documentation, Avatar Component Documentation, Badge Component Documentation, Breadcrumb Component Documentation, Button Component Documentation, Card Component Documentation, Carousel Component Documentation, Checkbox Component Documentation (+8 more)

### Community 4 - "Type Utilities & Config"
Cohesion: 0.20
Nodes (12): Table, Tag, Tooltip, Tree, TreeSelect, Typography, Arco Design Migration, Controlled/Uncontrolled Pattern (+4 more)

### Community 5 - "Icon-to-Component Wiring"
Cohesion: 0.27
Nodes (10): component-library-tooling capability, docs-tooling-simplification capability, migrate-web-vue-to-vite-plus design, migrate-web-vue-to-vite-plus proposal, migrate-web-vue-to-vite-plus tasks, packages/sd-vue-scripts, simplify-build-tooling-remove-sd-vue-scripts change, simplify-build-tooling tasks (+2 more)

### Community 6 - "Color Picker System"
Cohesion: 0.24
Nodes (10): ellipsis component, Ellipsis Component Docs, Layout Component Docs, layout component, layout-header component, layout-sider component, Menu Component Docs, menu component (+2 more)

### Community 7 - "Anchor & Descriptions Components"
Cohesion: 0.28
Nodes (9): DemoEditor REPL preview, generated-demo-source-conventions spec, interactive-doc-examples spec (modified), migrate-generated-demos design, migrate-generated-demos-to-script-setup-tailwind, migrate-generated-demos tasks, script setup lang=ts convention, shared demo Tailwind stylesheet (+1 more)

### Community 8 - "JSON Form Engine"
Cohesion: 0.22
Nodes (9): skills_overdrive_browser_iteration, skills_overdrive_context_assessment, skills_overdrive_frontend_design_dependency, skills_overdrive_performance_rules, skills_overdrive_progressive_enhancement, skills_overdrive_propose_before_build, skills_overdrive_skill, skills_overdrive_toolkit (+1 more)

### Community 9 - "Input & Display Components"
Cohesion: 0.29
Nodes (8): Radio Component, TableColumnData, Table Component, Naive UI Compatibility Aliases, TreeSelect Component, Tree Component, TreeNodeData, VirtualListProps (Tree)

### Community 10 - "Date Picker & Panels"
Cohesion: 0.25
Nodes (8): Generated Demo Source Conventions Spec, Migrate Generated Demos to Script Setup + Tailwind, sd-vue-docs, Astro Config (sd-vue-docs), Generated Docs Sidebar, LLMs Documentation Integration, LLMs.txt Documentation Page, Tailwind CSS Setup

### Community 11 - "Avatar/Badge/Hooks"
Cohesion: 0.29
Nodes (7): skills_polish_checklist, skills_polish_dimensions, skills_polish_final_verification, skills_polish_frontend_design_dependency, skills_polish_interaction_states, skills_polish_pre_assessment, skills_polish_skill

### Community 12 - "Ellipsis & Utility Hooks"
Cohesion: 0.40
Nodes (5): skills_quieter_frontend_design_dependency, skills_quieter_intensity_assessment, skills_quieter_philosophy, skills_quieter_refinement_dimensions, skills_quieter_skill

### Community 13 - "Agent/Skill Evaluation"
Cohesion: 0.40
Nodes (5): skills_typeset_assessment, skills_typeset_frontend_design_dependency, skills_typeset_improvement_dimensions, skills_typeset_sizing_strategy, skills_typeset_skill

### Community 14 - "Form & Tabs System"
Cohesion: 0.50
Nodes (4): Image Component Doc, Popconfirm Component Doc, Popover Component Doc, Trigger Component

### Community 15 - "Menu System"
Cohesion: 0.67
Nodes (3): Build Artifacts (es, dist, json), Public Vendor Resources, sync-vendor.mjs

### Community 16 - "Input/Tag/Ellipsis Utilities"
Cohesion: 0.67
Nodes (3): CSS Variables Convention, SdThemeConfig, ThemeProvider

## Knowledge Gaps
- **58 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Should `Icon System` be split into smaller, more focused modules?**
  _Cohesion score 0.10952380952380952 - nodes in this community are weakly interconnected._
- **Should `Component Entry Points & Icons` be split into smaller, more focused modules?**
  _Cohesion score 0.13333333333333333 - nodes in this community are weakly interconnected._
- **Should `Layout & Panel Components` be split into smaller, more focused modules?**
  _Cohesion score 0.14619883040935672 - nodes in this community are weakly interconnected._